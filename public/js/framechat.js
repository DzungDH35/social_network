let framechatArea = document.getElementById('framechat-area');
...
function sendMsg(from, to, content) {
    // 1. return sent message if successful
    // 2. return a pending
    // 3. return an error after being timed out (use a timer)
    return content;
}

// receive messages from socket
function receiveMsg(from, to, content) {
    return content;
}
/* ============================================================ */
...
function registerEventsInFramechat(contextID, userID) {
    ...
    msgInput.onkeypress = function(event) {
        if (event.keyCode == 13 && !event.shiftKey) {
            ...
            let sentMsg = sendMsg(contextID, userID, msgInput.value); msgInput.value = "";
            displaySentMessage(userID, sentMsg);
        }
    };
...
function generateFramechatDOMStr(userID, userName, userAvt) {
    return `<div class="framechat-wrapper">
                ...
                <div class="framechat-footer">
                    ...
                    <textarea rows="1" oninput='this.style.height = "";this.style.height = this.scrollHeight + "px"' class="framechat-footer__msg-input" placeholder="Text something..."></textarea>
                    <label class="framechat-footer__icon-wrapper">
